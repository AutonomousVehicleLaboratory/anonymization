...
def get_xyz_intensity_points(cloud_array, remove_nans=True):
    '''
    Pulls out x, y, and z columns from the cloud recordarray, and returns a 3xN matrix.
    '''
    # remove crap points
    ...
    # pull out x, y, and z values
    points = np.zeros(list(cloud_array.shape) + [4], dtype=np.float)
    points[...,0] = cloud_array['x']
    points[...,1] = cloud_array['y']
    points[...,2] = cloud_array['z']
...
def pointcloud2_to_xyz_intensity_array(cloud_msg, remove_nans=True):
    """
    super fast point cloud message extraction
    https://github.com/dlaz/pr2-python-minimal/blob/3fc1dad91d2029ddc62c0b8303c895545ea687f4/src/pr2_python/pointclouds.py
    """
    return get_xyz_intensity_points(pointcloud2_to_array(cloud_msg), remove_nans=remove_nans)
